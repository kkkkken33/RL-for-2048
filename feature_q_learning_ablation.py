...
class FeatureExtractor:
	...
	def __init__(self, enabled_features: dict[str, bool] | None = None) -> None:
	...
	def phi(self, board: np.ndarray, action: int) -> tuple[np.ndarray, bool]:
		...
		features = np.array([
			1.0,
			0.0,
			self._count_empty(after_board) if self.enabled_features["empty_ratio"] else 0.0,
			self._max_tile_feature(after_board) if self.enabled_features["max_tile"] else 0.0,
			self._monotonicity(after_board) if self.enabled_features["monotonicity"] else 0.0,
			self._smoothness(after_board) if self.enabled_features["smoothness"] else 0.0,
			self._merge_potential(after_board) if self.enabled_features["merge_potential"] else 0.0,
			(merge_score / 2048.0) if self.enabled_features["immediate_merge_score"] else 0.0,
			self._corner_max(after_board) if self.enabled_features["corner_max"] else 0.0,
		], dtype=np.float64)
...
def train(args: argparse.Namespace) -> None:
	...
	if args.disable_feature is not None:
		enabled_features[args.disable_feature] = False

	print(f"Enabled features: {enabled_features}")

	extractor = FeatureExtractor(enabled_features=enabled_features)
	agent = LinearQAgent(
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		...
		choices=[
			...
			"smoothness",
			"merge_potential",
			"immediate_merge_score",
			"corner_max",
		],
		help="Disable one feature for ablation study",
	)
